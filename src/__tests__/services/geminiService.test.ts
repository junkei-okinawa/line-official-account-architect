...
describe('chatWithGemini', () => {
  ...
  it('should throw MISSING_API_KEY error when apiKey is empty', async () => {
    ...
    await expect(chatWithGemini(mockMessages, mockSettings, '')).rejects.toHaveProperty(
      'code',
      'MISSING_API_KEY'
    );
  });
  ...
  it('should throw INVALID_API_KEY error when API returns 401', async () => {
    ...
    await expect(chatWithGemini(mockMessages, mockSettings, 'invalid-key')).rejects.toThrow(
      GeminiError
    );
    await expect(chatWithGemini(mockMessages, mockSettings, 'invalid-key')).rejects.toHaveProperty(
      'code',
      'INVALID_API_KEY'
    );
  });
  ...
  it('should throw RATE_LIMITED error when API returns 429', async () => {
    ...
    await expect(chatWithGemini(mockMessages, mockSettings, 'valid-key')).rejects.toHaveProperty(
      'code',
      'RATE_LIMITED'
    );
  });
